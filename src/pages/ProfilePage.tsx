...
export default function ProfilePage() {
  ...
  const { data: profileData } = useProfile(authProfile?.id || '');
  ...
  // Type guard for profile with extended fields
  const getProfileField = (field: string): string | null | undefined => {
    if (!profile) return undefined;
    return (profile as Record<string, unknown>)[field] as string | null | undefined;
  };
            ...
            {editingField === 'jobTitle' ? (
              ...
              <>
                <div className='flex items-center gap-3 flex-1'>
                  <Briefcase className='w-4 h-4 text-gray-400' />
                  <span className={`text-sm ${getProfileField('jobTitle') ? 'text-gray-700' : 'text-gray-400'}`}>
                    {getProfileField('jobTitle') || 'Your job title'}
                  </span>
                </div>
                <button
                  onClick={() => handleEditStart('jobTitle', getProfileField('jobTitle'))}
          ...
          <div className='flex items-center justify-between py-2'>
            {editingField === 'department' ? (
              ...
              <>
                <div className='flex items-center gap-3 flex-1'>
                  <Building2 className='w-4 h-4 text-gray-400' />
                  <span className={`text-sm ${getProfileField('department') ? 'text-gray-700' : 'text-gray-400'}`}>
                    {getProfileField('department') || 'Your department'}
                  </span>
                </div>
                <button
                  onClick={() => handleEditStart('department', getProfileField('department'))}
                  className='text-the-dark-blue hover:text-the-dark-blue/80'
                >
                  <Pencil className='w-3.5 h-3.5' />
                </button>
          ...
          <div className='flex items-center justify-between py-2'>
            {editingField === 'organization' ? (
              ...
              <>
                <div className='flex items-center gap-3 flex-1'>
                  <Building2 className='w-4 h-4 text-gray-400' />
                  <span className={`text-sm ${getProfileField('organization') ? 'text-gray-700' : 'text-gray-400'}`}>
                    {getProfileField('organization') || 'Your organization'}
                  </span>
                </div>
                <button
                  onClick={() => handleEditStart('organization', getProfileField('organization'))}
                  className='text-the-dark-blue hover:text-the-dark-blue/80'
                >
                  <Pencil className='w-3.5 h-3.5' />
                </button>
          ...
          <div className='flex items-center justify-between py-2'>
            {editingField === 'location' ? (
              ...
              <>
                <div className='flex items-center gap-3 flex-1'>
                  <MapPin className='w-4 h-4 text-gray-400' />
                  <span className={`text-sm ${getProfileField('location') ? 'text-gray-700' : 'text-gray-400'}`}>
                    {getProfileField('location') || 'Your location'}
                  </span>
                </div>
                <button
                  onClick={() => handleEditStart('location', getProfileField('location'))}
                  className='text-the-dark-blue hover:text-the-dark-blue/80'
                >
                  <Pencil className='w-3.5 h-3.5' />
                </button>
        ...
        <div className='bg-white border border-gray-200 rounded-lg p-4'>
          <div className='flex items-center justify-between py-2'>
            <div className='flex items-center gap-3 flex-1'>
              ...
              <span className='text-sm text-gray-700'>{profile?.email || 'email@example.com'}</span>
            </div>
          </div>
        </div>
      </div>
